...
def handle_keys(raw_key):
    global GN_PROBABILITY_MIN, ty_proc, gn_proc, shift_down
...
def main():
    ...
    # Set logging level and for only errors
    mvnc.global_set_option(mvnc.GlobalOption.RW_LOG_LEVEL, 3)
    devices = mvnc.enumerate_devices()
    if (len(devices) < 1):
        print('This application requires at least 1 NCS device.')
        print('Insert at least one NCS device and try again!')
        return 1

    ty_device = mvnc.Device(devices[0])
    ty_device.open()

    running_on_one_device = False

    if (len(devices) == 1):
        # Only one device in system, use same device for both networks.
        running_on_one_device = True
        print("Only one NCS device detected, running on one device.")
        gn_device = ty_device
    else:
        print("Running on two devices")
