...
def global_avg_pool(x, data_format=INVALID):
    ...
    if data_format == 'NHWC':
        ...
        return tf.layers.flatten(tf.layers.average_pooling2d(x, [x.shape[1], x.shape[2]], [x.shape[1], x.shape[2]]))
    else:
...
def factorized_reduction(net, output_filters, stride, data_format=INVALID):
    ...
    # note: slicing not supported on NCS so we're skipping it
    if data_format == 'NHWC':
    ...
    # TODO modified nasnet to fuse BN by placing before concat
    # Concat and apply BN
    path1_bn = slim.batch_norm(path1, scope='path1_bn')
    path2_bn = slim.batch_norm(path2, scope='path2_bn')
    final_path = tf.concat(values=[path1_bn, path2_bn], axis=concat_axis)
    return final_path
...
def _stacked_separable_conv(net, stride, operation, filter_size):
    ...
    for layer_num in range(num_layers - 1):
        ...
        if net.shape[3] != filter_size:
            # Do a 3x3 reduction before applying the conv due to NCS limitations
            net = tf.layers.conv2d(net, filter_size, 3, name=f'reduction_3x3_sep', padding='SAME')
