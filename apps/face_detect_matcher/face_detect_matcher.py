#! /usr/bin/env python3
# Contributing authors: Neal Smith <neal.p.smith@intel.com>, Tome Vang <tome.vang@intel.com>
...
validated_image_filename = VALIDATED_IMAGES_DIR + 'valid.png'
...
# higher number = larger rectangle around detected faces
FACE_PADDING = 50

# how often to send an image for inference
INFERENCE_INTERVAL = 1.0

# text and rectangle colors
BLUE = (255, 0, 0)
GREEN = (0, 255, 0)
RED = (0, 0, 255)
...
# returns None
def overlay_on_image(vid_image, found_match, x, y, w, h):
    if found_match:
        color = GREEN
    else:
        color = RED

    cv2.rectangle(vid_image, (x - FACE_PADDING, y - FACE_PADDING),
                              (x + w + FACE_PADDING, y + h + FACE_PADDING), color, 2)
...
def run_camera(valid_output, validated_image_filename, graph):
    ...
    while True :
        ...
        # detect faces
        faces = face_cascade.detectMultiScale(vid_image, 1.05, 5)

        # Draw rectangle over each face, make an inference, and do a face comparison vs the validated image
        for (x, y, w, h) in faces:
			# makes sure face is in window
            if x - FACE_PADDING > 0 and y - FACE_PADDING > 0 and x + w + FACE_PADDING < REQUEST_CAMERA_WIDTH and y + y + FACE_PADDING < REQUEST_CAMERA_HEIGHT:
				# crops face
                cropped_image = vid_image[y - FACE_PADDING:y + h + FACE_PADDING, x - FACE_PADDING:x + w + FACE_PADDING]

                if (VIEW_CROPPED_IMAGE):
                    cv2.imshow("Cropped face", cropped_image)
                
                if time_to_make_inference:
                    test_output = run_inference(cropped_image, graph)
                    ...
                    time_to_make_inference = False
                    start_time = time.time()
                       
                overlay_on_image(vid_image, found_match, x, y, w, h)
                    
        # check inference timer
        ...
        if VIEW_VALID_IMAGE:		
            validated_image = cv2.imread(validated_image_filename,1)
            cv2.imshow("Valid Image", validated_image)
