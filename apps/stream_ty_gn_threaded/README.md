...
# How it Works
One or two NCS devices are needed to run this application.  When two devices are detected, one executes inferences for the Tiny Yolo network and one executes inferences for the googlenet network.  When only one device is detected, both networks execute on the same device and performance will be noticeably slower.  OpenCV is used to open a USB camera which is assumed to be attached to the host computer.  For each frame of video that the camera returns the program first runs a Tiny Yolo inference to find all objects in the image.  This will be limited to the 20 categories that Tiny Yolo recognizes.  Those categories can be seen in the code in this line:
