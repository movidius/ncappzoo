...
def show_countdown(frame, emotionLabel):
    ...
    smile = cv2.addWeighted(frame[5:55, 5:55, :], 0.1, emoji, 1, 0)
    frame[5:55, 5:55] = smile

    if totalSecond >= minSecond:
        if totalSecond <= 5:
            cv2.putText(frame, str(totalSecond), (int(15), int(90)), cv2.FONT_HERSHEY_DUPLEX, 1, GREEN_COLOR, 2, 1)
...
def main():
    ...
    face_model_xml = os.getcwd() + "/src/data/face-detection/face-detection-adas-0001.xml"
    ...
    while cap.isOpened():
        ...
        cv2.putText(img, 'High score: {}'.format(high_score), (FRAME_WIDTH - 250, 30), cv2.FONT_HERSHEY_DUPLEX, 0.7,
                    RED_COLOR, 1, 1)
        cv2.putText(img, 'Your score is: {}'.format(score), (60, 30), cv2.FONT_HERSHEY_DUPLEX, 0.7, DARK_GREEN_COLOR, 1, 1)
        picked_emoji = show_countdown(img, emotionLabel)

        prepimg = cv2.resize(img, (672, 384))
        ...
        for detection in res[0][0]:
            ...
            if confidence > 0.7:
                ...
                if ymin is not 0 and ymax is not 0:
                    ...
                    cv2.putText(img, emotion_text, (abs(xmin), abs(ymin - 10)), cv2.FONT_HERSHEY_DUPLEX, 0.7, YELLOW_COLOR, 1, 1)
        ...
        if args.show_fps:
            ...
            cv2.putText(img, fps, (15, FRAME_HEIGHT - 50), cv2.FONT_HERSHEY_SIMPLEX, 0.4, YELLOW_COLOR, 1, cv2.LINE_AA)
