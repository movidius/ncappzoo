...
// graph file name - assume we are running in this directory: ncsdk/examples/caffe/ResNet-18/cpp
#define GRAPH_FILE_NAME "../graph"

// image file name - assume we are running in this directory: ncsdk/examples/caffe/ResNet-18/cpp
#define IMAGE_FILE_NAME "../../../data/images/nps_electric_guitar.png"


// ResNet-18 image dimensions, network mean values for each channel in BGR order.
const int networkDim = 224;
float networkMean[] = {103.0626238, 115.90288257, 123.15163084};
...
int main(int argc, char** argv)
{
    ...
    retCode = ncGraphCreate("ResNet-18 Graph", &graphHandlePtr);
    if (retCode != NC_OK)
    ...
    {   // successfully created graph.  Now we need to destory it when finished with it.
        ...
        else
        {
            ...
            unsigned int lenBufFp32 = 0;
            float* imageBufFP32Ptr = LoadImage32(IMAGE_FILE_NAME, networkDim, networkMean, &lenBufFp32);
            ...
            retCode = ncGraphQueueInferenceWithFifoElem(
                          graphHandlePtr, inFifoHandlePtr, outFifoHandlePtr, imageBufFP32Ptr, &lenBufFp32, NULL);
            if (retCode != NC_OK)
